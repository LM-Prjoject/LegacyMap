...
export const ChatWidget = () => {
        ...
        prevRooms.map(room =>
          room.id === roomId
            ? {
              ...room,
              members: room.members.map(m =>
                m.userId === currentUserId ? { ...m, muted: currentMuted } : m
              )
            }
            : room
        )
  ...
  }, [currentRoom, currentUserId, isRoomMuted, setRooms]);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !editingMessageRef.current.contains(event.target as Node)
      ) {
  ...
  const handleChangeMemberRole = async (newRole: ChatMemberRole) => {
    if (!currentRoom || !selectedMemberForRoleChange) return;

    const memberId = selectedMemberForRoleChange.userId;
    const oldRole = selectedMemberForRoleChange.role;
    ...
    setRooms(prev => prev.map(r =>
      r.id === currentRoom.id
        ? {
          ...r,
          members: r.members.map(m => m.userId === memberId ? { ...m, role: newRole } : m)
        }
        : r
    ));
    setSelectedMemberForRoleChange(null);
    showToast.success('Đã cập nhật vai trò thành viên');
    ...
    try {
      ...
      setRooms(prev => prev.map(r =>
        r.id === currentRoom.id
          ? {
            ...r,
            members: r.members.map(m => m.userId === memberId ? { ...m, role: oldRole } : m)
          }
          : r
      ));
  ...
  const handleEditMessage = useCallback(
    (messageId: string) => {
      ...
      // No await, handled optimistically in context
      updateMessage(currentRoom.id, messageId, { messageText: editingText.trim() });
      setEditingMessageId(null);
      setEditingText('');
    },
  ...
  const handleDeleteMessage = useCallback(
    (messageId: string, isAdmin = false) => {
      if (!currentRoom) return;
      // No await, handled optimistically in context
      deleteMessage(currentRoom.id, messageId, isAdmin);
    },
              ...
              <div className="max-h-64 overflow-y-auto divide-y divide-white/5">
                {currentRoom.members.map((member) => {
                  ...
                  return (
                    <div
                      key={member.userId}
                      className="px-4 py-2.5 text-sm flex items-center justify-between hover:bg-white/5 transition"
                    >
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-full font-medium ${isAdmin
                        ...
                        >
                          {isAdmin ? 'Admin' : isModerator ? 'Điều phối' : 'Thành viên'}
                        </span>

                        {(() => {
                          if (!currentRoom || !currentUserId) return null;
                          const myMember = currentRoom.members.find(m => m.userId === currentUserId);
                          if (myMember?.role !== 'admin') return null;
                          if (member.userId === currentUserId) return null;
                          if (!['branch', 'group'].includes(currentRoom.roomType)) return null;
                          ...
                          return (
                            <button
                              ...
                              className="text-xs text-[#ffd89b]/80 hover:text-[#ffd89b] underline transition"
                            >
                              Đổi vai trò
                            </button>
                          );
                ...
                {currentMessages.map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} group`}>
                      <div ref={editingMessageId === message.id ? editingMessageRef : null}
  ...
  return (
    <>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  onClick={async () => {
                    ...
                    const oldNickname = currentRoom.members.find(m => m.userId !== currentUserId)?.nickname;
                    ...
                    showToast.success(`Đã đổi biệt danh thành: ${newNickname}`);
                    setShowNicknameModal(false);
                    setTempNickname('');
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    // No await, handled optimistically in context
                    updateRoom(currentRoom.id, { name: newName });
                    setShowEditBranchNameModal(false);
                    setTempBranchName('');
                  }}
                  className="px-5 py-2.5 rounded-xl bg-[#ffd89b] hover:bg-[#ffd89b]/90 text-[#1e2a3a] font-medium text-sm transition shadow-lg"
      ...
      {selectedMemberForRoleChange && currentRoom && (
        <div className="fixed inset-0 z-[1400] bg-black/50 flex items-center justify-center px-4">
          <div className="bg-[#1e2a3a] rounded-2xl border border-[#ffd89b]/20 p-6 w-full max-w-sm shadow-2xl">
            <h3 className="text-lg font-semibold text-[#ffd89b] mb-4 text-center">
              Đổi vai trò cho {selectedMemberForRoleChange.username}
            </h3>
              ...
              {(['member', 'moderator', 'admin'] as const).map(role => (
                <label key={role} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="radio"
                    name="role"
                    checked={selectedMemberForRoleChange.role === role}
                    onChange={() => handleChangeMemberRole(role)}
                    className="w-4 h-4"
                    style={{ accentColor: 'rgb(255, 216, 155)' }}
                  />
                  <span className="text-white">
                    {role === 'admin' && 'Quản trị viên (Admin)'}
                    {role === 'moderator' && 'Điều phối viên (Moderator)'}
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-[#ffd89b]/10 hover:bg-[#ffd89b]/20 text-sm"
              >
                Hủy
              </button>
            </div>
